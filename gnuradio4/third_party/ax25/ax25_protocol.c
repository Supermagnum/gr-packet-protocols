...
int ax25_parse_frame(const uint8_t* data, uint16_t length, ax25_frame_t* frame) {
    ...
    while (pos < length - 2 && frame->num_addresses < AX25_MAX_ADDRS) {
        ...
        frame->addresses[frame->num_addresses].has_been_repeated = false;
        
        pos += 7;
        frame->num_addresses++;
        
        // H bit (LSB of address byte): 0 => another address follows, 1 => last address field
        if ((frame->addresses[frame->num_addresses - 1].ssid & 0x01U) != 0) {
            break;
        }
    }
...
AX25_EXPORT int ax25_encode_frame(const ax25_frame_t* frame, uint8_t* data, uint16_t* length) {
    ...
    for (uint8_t i = 0; i < frame->num_addresses; i++) {
        ...
        if (i == frame->num_addresses - 1) {
            data[pos + 6] |= 0x01; // H bit: final address field
        } else {
            data[pos + 6] &= (uint8_t)~0x01U; // more address fields follow
        }
        pos += 7;
    }
